...
import { CreateUpdatePurchaseTransactionFormDialog } from '@/components/PurchaseTransactionFormDialog';
import { Dropdown } from '@/components/Dropdown';

import { getSuppliers } from '@/service/supplier';
import { getInventories } from '@/service/inventory';
import { createPurchaseTransaction, deletePurchaseTransactionById, getPurchaseTransactions, updatePurchaseTransaction } from '@/service/purchase_transaction';
import type { PurchaseTransactionCreatePayload, PurchaseTransactionData, PurchaseTransactionUpdatePayload } from '@/model/purchase_transaction';
...
export const PurchaseTransactionPage = () => {
    ...
    const { data: inventoryData, isLoading: isInventoriesLoading } = useQuery({
        queryKey: ['inventories-all-purchase'],
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            toast.success(`Transaksi pembelian berhasil diubah.`);
            queryClient.invalidateQueries({ queryKey: ['purchase-transactions'] });
        },
        onError: (error) => { toast.error(error.message); },
    });
    ...
    const handleSave = async (data: PurchaseTransactionCreatePayload | PurchaseTransactionUpdatePayload) => {
        if (editingTransaction) {
            updateMutation.mutate({ ...data, id: editingTransaction.id });
        } else {
            createMutation.mutate(data as PurchaseTransactionCreatePayload);
        }
    };
    ...
    return (
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
            <PageHeading headingTitle={`Menu Pembelian`} actionButton={() => {}}/>
            <div className="bg-white dark:bg-gray-950 border p-4 rounded-xl shadow-sm mb-6">
                <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
                    <div>
                        ...
                        <Dropdown items={mapToDropdownItems(supplierData?.items, {valueKey: 'id', labelKey: 'name'})} value={supplierId} onChange={setSupplierId} placeholder='Pilih Supplier' isLoading={isSuppliersLoading} />
                    </div>
                     <div>
                        <Label className="block mb-2">Nama Kain</Label>
                    ...
                    <div className="flex flex-wrap gap-4 col-span-1 justify-start md:justify-end">
                        ...
                        <Button className="bg-blue-500 hover:bg-blue-600" onClick={openAddDialog}><Plus className="mr-2 h-4 w-4" />Tambah Data Pembelian</Button>
                    </div>
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="text-center p-8 text-gray-500 bg-gray-50 border rounded-xl shadow-sm">Data transaksi pembelian kosong.</div>
            ) : (
                <div>
                    <div className="bg-white dark:bg-gray-950 border rounded-xl shadow-sm overflow-hidden hidden md:block">
                        <Table>
                            ...
                            <TableFooter><TableRow><TableCell colSpan={6} className="font-bold text-right py-4">Total Pembelian</TableCell><TableCell className="text-right font-bold">{formatCurrency(transactions.reduce((sum, r) => sum + r.total, 0))}</TableCell><TableCell/></TableRow></TableFooter>
                        </Table>
                    </div>
                        ...
                        {transactions.map((data) => (
                            <Card key={data.id}>
                                ...
                                <CardContent className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    ...
                                    <div className="font-semibold text-gray-500">Roll</div><div className="text-right">{formatNumber(data.roll_count)}</div>
                                    <div className="font-semibold text-gray-500">Berat (Kg)</div><div className="text-right">{formatNumber(data.weight_kg)}</div>
            ...
            {isFormOpen && (
                <CreateUpdatePurchaseTransactionFormDialog
